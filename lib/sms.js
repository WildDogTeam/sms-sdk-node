...
var SMS = function (settings) {
    if (!(_.isObjectLike(settings) && _.has(settings, 'appId') && _.has(settings, 'smsKey'))) {
        throw new Error('first argument must be an object with appId and smsKey properties.');
    }
    this.appId = settings.appId;
    this.smsKey = settings.smsKey;

    this.API_URL_PREFIX = 'https://sms.wilddog.com/api/v1/';
...
SMS.prototype.__calcSignature__ = function (form_data) {
    //sort keys
    var keys = _.keysIn(form_data);
    var sortedKeys = _.sortBy(keys);
    var sortedFormData = {};
    _.forEach(sortedKeys, function (k) {
        sortedFormData[k] = form_data[k];
    });
    var param_str = decodeURIComponent(querystring.stringify(sortedFormData));
    param_str += '&';
    param_str += this.smsKey;
    console.log(param_str);

    return crypto.Hash('sha256').update(param_str, 'utf-8').digest('hex');
};
